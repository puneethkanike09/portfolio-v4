...
const RotatingText = forwardRef<RotatingTextRef, RotatingTextProps>((props, ref) => {
    ...
    const safeTexts = Array.isArray(texts) ? texts : [String(texts)];
    ...
    const elements = useMemo(() => {
        ...
        if (splitBy === "words") {
            return String(currentText).split(" ").map((word, i, arr) => ({
                characters: [word],
                needsSpace: i !== arr.length - 1,
            }));
        }
        if (splitBy === "lines") {
            return String(currentText).split("\n").map((line, i, arr) => ({
                characters: [line],
                needsSpace: i !== arr.length - 1,
            }));
        }
        return String(currentText).split(splitBy).map((part, i, arr) => ({
            characters: [part],
            needsSpace: i !== arr.length - 1,
        }));
